...
static void sec_bat_siop_work(struct work_struct *work)
{
	...
	if (siop_level < 100) {
		...
		if (chg->cp_disable_votable)
			vote(chg->cp_disable_votable, SEC_BATTERY_SIOP_VOTER, true, 0);
		else
		...
		if (chg->smb_override_votable)
			vote(chg->smb_override_votable, SEC_BATTERY_SIOP_VOTER, true, 0); 
		else
		...
		if (!chg->smb_override_votable)
			chg->smb_override_votable = find_votable("SMB_EN_OVERRIDE");
		if (chg->smb_override_votable)
			vote(chg->smb_override_votable, SEC_BATTERY_SIOP_VOTER, false, 0); 
		else
			pr_err("Couldn't find chg->smb_override_votable\n");
		...
		if (chg->cp_disable_votable)
			vote(chg->cp_disable_votable, SEC_BATTERY_SIOP_VOTER, false, 0);
		else
			pr_err("Couldn't find chg->cp_disable_votable\n");
	}
...
static void sec_bat_monitor_work(struct work_struct *work)
{
	...
	const char *usb_icl_voter, *chg_disable_voter, *fcc_voter, *cp_disable_voter = NULL;
	...
	if (battery->psy_slave) {
		...
		if (rc < 0) {
			dev_err(battery->dev, "%s: Fail to get cp die temp prop. rc=%d\n", __func__, rc);
	...
	chg_disable_voter = get_effective_client(chg->chg_disable_votable);
	fcc_voter = get_effective_client(chg->fcc_votable);
	if(!!chg->cp_disable_votable)
		cp_disable_voter = get_effective_client(chg->cp_disable_votable);
	
	pr_info("%s: Status(%s, %s), Health(%s), Cable(%s, %s, %s), Afc Status(%d)\n", __func__,
			...
			battery->conn_type ? "MICRO USB" : "TYPE C",
			sec_bat_typec_mode_str[battery->typec_mode], chg->afc_sts);
	...
	pr_info("%s: usb_icl_votable(%s): %d, chg_disable_votable(%s): %d\n", __func__,
			usb_icl_voter? usb_icl_voter : "None", get_effective_result(chg->usb_icl_votable),
			chg_disable_voter? chg_disable_voter : "None", get_effective_result(chg->chg_disable_votable));
	pr_info("%s: fcc_votable(%s): %d, cp_disable_votable(%s): %d\n", __func__,
			fcc_voter? fcc_voter : "None", get_effective_result(chg->fcc_votable),
			cp_disable_voter? cp_disable_voter : "None", (!chg->cp_disable_votable)? 0 : get_effective_result(chg->cp_disable_votable));
