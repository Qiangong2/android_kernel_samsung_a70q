...
#define ENDING_SPEED	10416

#define STARTING_SPEED_LOW_TEMP	1052631
#define FULL_SPEED_LOW_TEMP	400000
#define ENDING_SPEED_LOW_TEMP	400000
...
#define TIMEOUT_TIME		900	/* starting time + full speed time + ending time */

#define FULL_SPEED_TIME_LOW_TEMP	100	/* starting time */
#define ENDING_TIME_LOW_TEMP		1165	/* starting time + full speed time*/
#define TIMEOUT_TIME_LOW_TEMP		1225	/* starting time + full speed time + ending time */
...
static int stspin220_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pdata->pinctrl)) {
		...
		pdata->pin_disable = pinctrl_lookup_state(pdata->pinctrl, "pm_gpio6_pwm_disable");
