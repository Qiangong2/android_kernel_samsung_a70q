...
bool get_is_usb(void)
{
	...
	pr_info("%s: is_usb : %d, is_usb_device_connected : %d\n",
		__func__, typec_manager.is_usb, is_usb_device_connected);
	return typec_manager.is_usb | is_usb_device_connected;
#else
	pr_info("%s: is_usb : %d\n",
		__func__, typec_manager.is_usb);
		...
		unsigned long action, void *data)
{
	...
	case EXTERNAL_NOTIFY_DEVICEADD:
		...
		if (enable && 
			...
			/* USB Cable type */
			typec_manager.is_usb_device_connected = true;
		} else if (!enable)
			typec_manager.is_usb_device_connected = false;
		break;
	default:
		break;
	}
