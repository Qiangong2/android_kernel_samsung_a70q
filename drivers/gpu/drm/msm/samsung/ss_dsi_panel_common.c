...
/* SAMSUNG_FINGERPRINT */
static void ss_wait_for_te_gpio(struct samsung_display_driver_data *vdd, int us)
{
	unsigned int disp_te_gpio;
	int rc, te_count = 0;
...
skip_bl_update:
	/*++ SAMSUNG_FINGERPRINT ++*/
	/* hbm needs 2 TE to be updated, where as normal needs no */
	if (backlight_origin == BACKLIGHT_FINGERMASK_ON) {
		ss_wait_for_te_gpio(vdd, 200);
		ss_wait_for_te_gpio(vdd, 200);
	}
	if (backlight_origin >= BACKLIGHT_FINGERMASK_ON) {
