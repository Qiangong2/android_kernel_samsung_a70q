...
int afe_get_spk_r0(void)
{
	return this_afe.prot_cfg.r0[SP_V2_SPKR_1];
}

int afe_get_spk_t0(void)
{
	return this_afe.prot_cfg.t0[SP_V2_SPKR_1];
}

int afe_get_spk_r0_r(void)
{
	return this_afe.prot_cfg.r0[SP_V2_SPKR_2];
}

int afe_get_spk_t0_r(void)
{
	return this_afe.prot_cfg.t0[SP_V2_SPKR_2];
}
...
int afe_get_spk_v_vali_sts(void)
{
	return this_afe.th_vi_v_vali_resp.param.status[SP_V2_SPKR_1];
}

int afe_get_spk_v_vali_r_sts(void)
{
	return this_afe.th_vi_v_vali_resp.param.status[SP_V2_SPKR_2];
}
		...
		struct afe_sp_th_vi_v_vali_get_param *th_vi_v_vali)
{
	...
	pr_info("afe_get_sp_th_vi_v_vali_data 0x%x \n", port);
	memset(&param_hdr, 0, sizeof(param_hdr));
...
{
	int ret = 0;
	struct audio_cal_type_sp_th_vi_ftm_cfg *cal_data;
					  ...
					  void *data)
{
	...
	if (!afe_get_sp_th_vi_v_vali_data(&th_vi_v_vali)) {
		for (i = 0; i < SP_V2_NUM_MAX_SPKRS; i++) {
			pr_err("%s: v-vali param status = %d\n",
				  __func__, th_vi_v_vali.param.status[i]);
			if (th_vi_v_vali.param.status[i] == V_VALI_IN_PROGRESS) {
				cal_data->cal_info.status[i] = -EAGAIN;
			} else if (th_vi_v_vali.param.status[i] == V_VALI_SUCCESS) {
				      ...
				      void *data)
{
	...
	if (mode == MSM_SPKR_PROT_IN_V_VALI_MODE)
		return afe_get_cal_sp_th_vi_v_vali_param(cal_type,
						data_size, data);
	else
		return afe_get_cal_sp_th_vi_ftm_param(cal_type,
						data_size, data);
}
