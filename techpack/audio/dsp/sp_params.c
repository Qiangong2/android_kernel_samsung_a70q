...
static ssize_t sp_count_exceeded_temperature_show(struct device *dev,
...
static DEVICE_ATTR(count_exceeded_temperature, 0644, sp_count_exceeded_temperature_show, NULL);
...
static DEVICE_ATTR(count_exceeded_temperature_r, 0644, sp_count_exceeded_temperature_r_show, NULL);

static ssize_t sp_count_exceeded_excursion_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	ssize_t ret = 0;
...
static DEVICE_ATTR(count_exceeded_excursion, 0644, sp_count_exceeded_excursion_show, NULL);
...
static DEVICE_ATTR(count_exceeded_excursion_r, 0644, sp_count_exceeded_excursion_r_show, NULL);

static ssize_t sp_max_excursion_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	ssize_t ret = 0;
	...
	ex_val_frac = ex_q27/Q20;
	ex_val_frac = (ex_val_frac * 100)/Q7;
	ret = sprintf(buf, "%d.%02d\n", 0, ex_val_frac);
	this_afe_spk.xt_logging.max_excursion[SP_V2_SPKR_1] = 0;
	return ret;
}
static DEVICE_ATTR(max_excursion, 0644, sp_max_excursion_show, NULL);
...
static ssize_t sp_max_temperature_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	ssize_t ret = 0;
...
static DEVICE_ATTR(max_temperature, 0644, sp_max_temperature_show, NULL);

static ssize_t sp_max_temperature_rd_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%d\n", this_afe_spk.max_temperature[SP_V2_SPKR_1]/Q22);
}
static DEVICE_ATTR(max_temperature_rd, 0644, sp_max_temperature_rd_show, NULL);

static ssize_t sp_max_temperature_rd_r_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%d\n", this_afe_spk.max_temperature[SP_V2_SPKR_2]/Q22);
}
static DEVICE_ATTR(max_temperature_rd_r, 0644, sp_max_temperature_rd_r_show, NULL);
				       ...
				       const char *buf, size_t size)
{
	...
	if (!kstrtou32(buf, 0, &initial_cal)) {
		...
		if (initial_cal == afe_get_spk_initial_cal())
			...
			afe_set_spk_initial_cal(initial_cal);//this_afe.initial_cal = initial_cal;
	}
	return size;
}
				       ...
				       const char *buf, size_t size)
{
	...
	if (!kstrtou32(buf, 0, &v_vali_flag)) {
		...
		if (v_vali_flag == afe_get_spk_v_vali_flag())
			...
			afe_set_spk_v_vali_flag(v_vali_flag);//this_afe.v_vali_flag = v_vali_flag;
	}
	return size;
}
...
static ssize_t q6afe_spk_r0_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	return sprintf(buf, "%d\n", afe_get_spk_r0());
...
static DEVICE_ATTR(spk_r0, 0644, q6afe_spk_r0_show, NULL);

static ssize_t q6afe_spk_t0_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	return sprintf(buf, "%d\n", afe_get_spk_t0());
...
static DEVICE_ATTR(spk_t0, 0644, q6afe_spk_t0_show, NULL);
...
static ssize_t q6afe_spk_v_vali_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	return sprintf(buf, "%d\n", afe_get_spk_v_vali_sts());
}

static DEVICE_ATTR(spk_v_vali_status, 0644, q6afe_spk_v_vali_show, NULL);

static ssize_t q6afe_spk_v_vali_r_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	return sprintf(buf, "%d\n", afe_get_spk_v_vali_r_sts());
}
...
int afe_get_sp_xt_logging_data(u16 port_id)
{
	...
	if (this_afe_spk.xt_logging.max_temperature[SP_V2_SPKR_1] < xt_logging_data.max_temperature[SP_V2_SPKR_1])
		this_afe_spk.xt_logging.max_temperature[SP_V2_SPKR_1] = xt_logging_data.max_temperature[SP_V2_SPKR_1];


	if (this_afe_spk.xt_logging.max_temperature[SP_V2_SPKR_2] < xt_logging_data.max_temperature[SP_V2_SPKR_2])
		this_afe_spk.xt_logging.max_temperature[SP_V2_SPKR_2] = xt_logging_data.max_temperature[SP_V2_SPKR_2];

	/* update temp for max_temperature_rd node*/
	if (this_afe_spk.max_temperature[SP_V2_SPKR_1] <
		xt_logging_data.max_temperature[SP_V2_SPKR_1])
		this_afe_spk.max_temperature[SP_V2_SPKR_1] =
				xt_logging_data.max_temperature[SP_V2_SPKR_1];
	...
	if (this_afe_spk.xt_logging.max_excursion[SP_V2_SPKR_1] < xt_logging_data.max_excursion[SP_V2_SPKR_1])
		this_afe_spk.xt_logging.max_excursion[SP_V2_SPKR_1] = xt_logging_data.max_excursion[SP_V2_SPKR_1];

	if (this_afe_spk.xt_logging.max_excursion[SP_V2_SPKR_2] < xt_logging_data.max_excursion[SP_V2_SPKR_2])
		this_afe_spk.xt_logging.max_excursion[SP_V2_SPKR_2] = xt_logging_data.max_excursion[SP_V2_SPKR_2];

	if (this_afe_spk.xt_logging.count_exceeded_temperature[SP_V2_SPKR_1] < xt_logging_data.count_exceeded_temperature[SP_V2_SPKR_1])
		this_afe_spk.xt_logging.count_exceeded_temperature[SP_V2_SPKR_1] += xt_logging_data.count_exceeded_temperature[SP_V2_SPKR_1];

	if (this_afe_spk.xt_logging.count_exceeded_temperature[SP_V2_SPKR_2] < xt_logging_data.count_exceeded_temperature[SP_V2_SPKR_2])
		this_afe_spk.xt_logging.count_exceeded_temperature[SP_V2_SPKR_2] += xt_logging_data.count_exceeded_temperature[SP_V2_SPKR_2];

	if (this_afe_spk.xt_logging.count_exceeded_excursion[SP_V2_SPKR_1] < xt_logging_data.count_exceeded_excursion[SP_V2_SPKR_1])
		this_afe_spk.xt_logging.count_exceeded_excursion[SP_V2_SPKR_1] += xt_logging_data.count_exceeded_excursion[SP_V2_SPKR_1];

	if (this_afe_spk.xt_logging.count_exceeded_excursion[SP_V2_SPKR_2] < xt_logging_data.count_exceeded_excursion[SP_V2_SPKR_2])
		this_afe_spk.xt_logging.count_exceeded_excursion[SP_V2_SPKR_2] += xt_logging_data.count_exceeded_excursion[SP_V2_SPKR_2];
	return ret;
}
...
int __init spk_params_init(void)
{
	...
	if (this_afe_spk.p_class) {
		this_afe_spk.p_dev = device_create(this_afe_spk.p_class, NULL, 1, NULL, CLASS_NAME);
