...
correct_plug_type:
	...
	while (((mbhc->mbhc_cfg->detect_extn_cable) || (mbhc->slow_insertion)) && !time_after(jiffies, timeout)) {
		if (mbhc->hs_detect_work_stop) {
			pr_debug("%s: stop requested: %d\n", __func__,
					mbhc->hs_detect_work_stop);
		...
		if ((output_mv > WCD_MBHC_ADC_HS_THRESHOLD_MV) &&
		    (spl_hs_count < WCD_MBHC_SPL_HS_CNT) && (mbhc->slow_insertion == false)) {
		...
		if ((output_mv <= WCD_MBHC_ADC_HS_THRESHOLD_MV) &&
		    (!is_pa_on) && (mbhc->slow_insertion == false)) {
			/* Check for cross connection*/
