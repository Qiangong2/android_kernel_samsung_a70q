	...
	*val = (uint16_t)value;
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		case RDC_CAL:
			if ((mtpex > 0) && (otc > 0))
				value =
					tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_R25C);
			if (value < 0) {
				dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read R: %d\n",
					__func__, value);
				goto tfa2_read_calibrate_exit;
		...
		case F0_CAL:
			if ((mtpex > 0) && (otc > 0))
				value =
					tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_F0);
			if (value < 0) {
				dev_err(&tfa9xxx->i2c->dev, "%s: Unable to read F: %d\n",
					__func__, value);
				goto tfa2_read_calibrate_exit;
	...
	if (do_sync) {
		...
		list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
			...
			if (tfa9xxx_mute_control) {
				...
				tfa2_dev_set_state(tfa9xxx->tfa, TFA_STATE_MUTE);
			} else {
				tfa2_dev_set_state(tfa9xxx->tfa, TFA_STATE_UNMUTE);
			}
#else
...
#endif /* TFA_USE_GPIO_FOR_MCLK */
			usleep_range(MCLK_START_DELAY, MCLK_START_DELAY);
			dev_dbg(&drv->i2c->dev,
				"%s: wait until mclk is ready\n", __func__);
		}
	}
#endif /* TFA_CONTROL_MCLK */
